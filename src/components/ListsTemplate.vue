      ...
      <li v-for="(attr, name) in el" :key="name" class="list-group-item">
        <div v-if="typeof attr === 'object'">
          <div v-if="attr.type === 'u'">
            ...
            <div v-if="name === 'math'">
              <span class="delete-color" v-html="attr.oldValue"></span> &#8594;
              <span class="insert-color" v-html="attr.newValue"></span>
            </div>
            <div v-else>
              <span class="delete-color"> {{ attr.oldValue }} </span> &#8594;
              <span class="insert-color"> {{ attr.newValue }} </span>
            </div>
            <div v-if="attr.changeID" class="container">
              <decision-btn
                :changeID="attr.changeID"
                :d="decisionArr[attr.changeID]['decision']"
                @decision="this.updateDecision"
              />
            </div>
          </div>
        ...
        <div v-else>
          <span v-if="name === 'math'"
            ><b> unit: </b> <span v-html="attr"></span
          ></span>
          <span v-else>
            <b>{{ name }}: </b> {{ attr }}</span
          >
  ...
  methods: {
    currentSlide: {
      handler: function () {
        ...
        if (this.currentSlide == -1) this.currentSlide = length - 1;
        if (this.currentSlide >= length) this.currentSlide = 0;
      },
    },
