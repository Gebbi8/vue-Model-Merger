...
<template v-for="(node, index) in data" :key="index">
    <div class="card col-10" v-if="slideNum == index">
        <div class="card-body row g-0 p-0">
            ...
            <decision-btn :changeID="this.changeIds" :decisionArr="this.decisionArr" @decision="this.emitDecision"></decision-btn>
        </div>
    </div>
</template>
<div class="btn btn-primary col-1 arrow-btn" @click="slideNum++">
...
export default {
    ...
    methods: {
        getSlide: function () {
            ...
            console.debug(c, currentSlide, this.structuredData);
