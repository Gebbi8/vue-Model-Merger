<template>
  ...
  >
    <input
      ...
      :id="bV1"
      autocomplete="off"
      @click="this.emitDecision(0)"
      :checked="this.decision == 0"
    />
    ...
    <input
      ...
      :id="bV2"
      autocomplete="off"
      @click="this.emitDecision(1)"
      :checked="this.decision == 1"
    />
    <label class="btn btn-outline-primary" :for="`bV2`">V2</label>
  </div>
</template>
...
export default {
  ...
  mounted() {
    for (let n = 0; n < this.list.length; n++) {
      console.debug(this.list[n]);
    }
  },
  methods: {
    emitDecision: function (dec) {
      for (let n = 0; n < this.list.length; n++) {
        this.$emit("decision", this.list[n][0], dec);
      }
    },
  },
};
