...
export default {
  ...
  methods: {
    computeMerge: function () {
      ...
      for (const [id, a] of Object.entries(this.decisionArr)) {
        //s: changeId, decision (0/1/-1) but should not be -1 at this point, type of change (i,u,d)
      ...
      insertsToRemove.reverse().forEach((id) => {
    ...
    restoreDelete: function (id) {
      ...
      if (change.localName == "attribute") {
        ...
        this.newDoc
          .evaluate(path, this.newDoc, null, XPathResult.ANY_TYPE, null)
          .iterateNext()
      ...
      if (change.localName == "text") {
        ...
        this.newDoc
          .evaluate(parent, this.newDoc, null, XPathResult.ANY_TYPE, null)
          .iterateNext()
          .insertAdjacentText("beforeend", change.getAttribute("oldText"));
        return;
      }
      ...
      --- */
        ...
        let node = this.oldDoc
          .evaluate(oldPath, this.oldDoc, null, XPathResult.ANY_TYPE, null)
          .iterateNext();

        console.debug(appendPath, this.newDoc, node);
        if (
          this.newDoc
            .evaluate(appendPath, this.newDoc, null, XPathResult.ANY_TYPE, null)
            .iterateNext() == null
        ) {
          //insert parent list

          let parentNode = this.oldDoc
            .evaluate(appendPath, this.oldDoc, null, XPathResult.ANY_TYPE, null)
            .iterateNext();
          let parentPath = appendPath.substring(0, appendPath.lastIndexOf("/"));

          console.debug(parentPath, parentNode);
          this.newDoc
            .evaluate(parentPath, this.newDoc, null, XPathResult.ANY_TYPE, null)
            .iterateNext()
            .insertAdjacentElement("beforeend", parentNode.cloneNode(false));
        }

        this.newDoc
          .evaluate(appendPath, this.newDoc, null, XPathResult.ANY_TYPE, null)
          .iterateNext()
          .insertAdjacentElement("beforeend", node.cloneNode(true));
    ...
    --- */
      ...
      const path = useGetLocalXPath(change.attributes.newPath.value);

      if (change.localName == "node" || change.localName == "text") {
        //if a a delete was readded on the same node or text the iterateNext() + remove() will remove this correct element
        console.debug(path);
        //alert("?");
        this.newDoc
          .evaluate(path, this.newDoc, null, XPathResult.ANY_TYPE, null)
      ...
      if (change.localName == "attribute") {
        ...
        this.newDoc
          .evaluate(path, this.newDoc, null, XPathResult.ANY_TYPE, null)
          .iterateNext()
          .removeAttribute(change.getAttribute("name"));
        return;
      }

      if (change.localName != "node" || change.localName != "text")
        console.error("Unhadled remove insert: ", change.localName, change);
    },

    revertUpdate: function (id) {
      let change = this.getXmlLineById(id);
      if (change.localName != "attribute") {
        //dev check for type of elements that are affected
