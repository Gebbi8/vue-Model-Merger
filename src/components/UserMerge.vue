<template>
  <div id="user-merge">
    ...
    <div class="container">
      <local-files
        @file1="(f) => (this.file1 = f)"
        @file2="(f) => (this.file2 = f)"
      ></local-files>
    </div>
    ...
    <div v-if="view == 'model'">
      <ul class="list-group">
        <template v-for="(element, index) in modelArr" :key="index">
          <li
            v-if="index === 'modelAttr' || index === 'sbmlAttr'"
    ...
    <div v-else-if="view == 'species'" class="container row slider">
      <slider
        ...
        :decisionArr="this.decisionArr"
        :slideNumber="this.currentSpecies"
        @decision="this.updateDecision"
        @slideNum="(num) => (this.currentSpecies = num)"
      ></slider>
    </div>

    <div v-else-if="view == 'reactions'" class="container row slider">
      <slider
        :data="this.reactionsArr"
        :structuredData="this.structuredData"
        :decisionArr="this.decisionArr"
        :slideNumber="this.currentReaction"
        @decision="this.updateDecision"
        @slideNum="(num) => (this.currentReaction = num)"
      ></slider>
    </div>
  </div>
  ...
  <merger
    :decision-arr="decisionArr"
    :oldDoc="this.oldDocument"
    :newDoc="this.newDocument"
    :xmlDiff="this.xmlDiff"
  ...
  methods: {
    ...
    createInterface: function () {
      ...
      this.json.nodes.forEach((node) => {
        ...
        } else if (node.id.startsWith("s") && node.bivesChange != "nothing") {
          //want the subnetwork for the distance 2
    ...
    createInterfaceLocally: function (
      promiseV1,
      promiseV2,
      promiseDiff,
      promiseJson
    ...
    getModelChanges: function () {
      ...
      xmlLines.forEach((line) => {
        if (line.includes("triggeredBy="));
          ...
          (line.includes('/sbml[1]"') || line.includes('/sbml[1]/model[1]"')) &&
          type != null
        ) {
          ...
          type != null &&
          (line.includes("/listOfSpecies") || line.includes("/listOfReactions"))
        ) {
          //species and reaction changes are shown with DiVil, but we still need to add the changes to the decision array
    ...
    combineModelAttrWithChanges: function () {
      ...
      changes.forEach((c) => {
        ...
        if (
          path != undefined &&
          !path.includes("/notes") &&
          path != "/sbml[1]/model[1]/annotation[1]"
        ) {
      ...
      modelChanges.forEach((c) => {
        //handle possible targets, call functions for reusable code
        //target: attribute
      ...
      rulesChanges.forEach((c) => {
        ...
        if (c.target == "attribute") {
          ...
          c.target == "node" &&
          c.oldPath !== "/sbml[1]/model[1]/listOfRules[1]"
        ) {
          //TODO: target node
    ...
    addAttributeChanges: function (c, modelData, list) {
      ...
      if (c.type === "u") {
        ...
        //elements are already contained in newDoc -> mark inserted elements and add changeID
        if (list === "sbmlAttr" || list === "modelAttr")
    ...
    changedNode: function (c, modelData, list) {
      ...
      if (c.type == "i") {
        ...
        if (list === "listOfParameters")
          n = this.getSingleParameter(this.oldDocument, c.oldPath);
        else if (list === "listOfUnitDefinitions")
          n = this.getSingleUnit(this.oldDocument, c.oldPath);
    ...
    getMath: function (path, doc) {
      //from xmlParser think about getting it from divil
      // var path = regEx(line, "newPath");
      console.log(path, path.indexOf("/functionDefinition[1]/math[1]/"));

      if (path.indexOf("/functionDefinition[1]/math[1]/") != -1)
        path = path.substr(0, path.indexOf("/functionDefinition[1]/math[1]/"));
    ...
    getUnits: function (doc, path) {
      //produces Array out of information from Unit list of a doc
    ...
    getCompartments: function (doc, path) {
      // produces array of
      let n = getNode(doc, path);
      if (n == null) return null;
      ...
      for (let i = 0; i < compartments.length; i++) {
        ...
        for (let a = 0; a < compartments[i].attributes.length; a++) {
          comp[compartments[i].attributes[a].name] =
            compartments[i].attributes[a].value;
        }
    ...
    getSingleFunctionDefinition: function (doc, path) {
      //same structure as rules?
      path = useGetLocalXPath(path);
    ...
    getSingleRule: function (doc, path) {
      //same structure as unit but not for math
      path = useGetLocalXPath(path);
